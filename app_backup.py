...
def process_payment():
    """Iniciar proceso de pago contactless en el TPV"""
    data = request.get_json()
    if not data or 'door_id' not in data:
    ...
    try:
        ...
        if response['success']:
            ...
            logger.info(f"Pago inicializado en TPV - Payment ID: {response['payment_id']}")
            ...
            logger.error(f"Error inicializando pago en TPV: {response.get('error', 'Error desconocido')}")
            return jsonify({
                'success': False,
                'status': 'init_error',
...
def check_payment_status():
    ...
    if not data or 'payment_id' not in data:
        return jsonify({'success': False, 'error': 'Payment ID requerido'}), 400
            ...
            if status == 'approved':
                ...
                if door_id:
                    ...
                    if product:
                        ...
                        # Actualizar stock
                        db_manager.update_product_stock(product['id'], product['stock'] - 1)
                        
                        # Dispensar producto (abrir puerta)
                        hardware_success = hardware_controller.open_door(door_id)
                        
                        return jsonify({
                            'success': True,
                            'status': 'approved',
                            'message': 'Pago aprobado - producto dispensado',
                            'transaction_id': transaction_id,
                            'amount': product['price'],
                            'door_id': door_id,
...
def process_purchase(slot: str, payment_method: str, payment_id: str = None) -> dict:
    ...
    try:
        # Obtener información del producto
        product = db_manager.get_product_by_door(slot)
        if not product:
        ...
        if response['success'] and response['status'] == 'approved':
            ...
            transaction_id = response.get('transaction_id', f"TXN_{slot}_{int(time.time())}")
            db_manager.add_transaction({
                'door_id': slot,
                'product_id': product['id'],
                'amount': amount,
            ...
            # Dispensar producto
            hardware_controller.open_door(slot)
            ...
            return {
                'success': False,
                'error': response.get('error', 'Pago rechazado')
            }
